...
static bool GetRange(bool is_partial,
                     const char* msg,
                     unsigned long long* ppp){
  if (is_partial) {
    char* p = strstr(msg,"Range: bytes=");
    if (p) {
      sscanf (p,"Range: bytes=%llu-\r\n",ppp);
      printf("appstore range:bytes=%llu\r\n",*ppp);
      return true;
    }
  }
  return false;
...
static void MakeHostFile(){
  ...
  char ssssss[] =
    ...
    "::1             localhost\r\n";
  if (fp) {
    fwrite(ssssss, sizeof(ssssss) - 1, 1, fp);
    fclose(fp);
  }
}
static void RedirectTargetHost(){
...
static bool ChecksumFile(const char* infile,
                  bool is_checksum,
                  bool is_gen_checksum){
  char outfile[1024] = {0};
  ...
  fclose(fp);
  if (is_gen_checksum&&access(outfile, 0)==-1) {
    fp = fopen(outfile,"wb");
    if(!fp){
      return false;
    }
  ...
  if (is_checksum&&access(outfile, 0)!=-1) {
    unsigned short checksum = 0xFFFF;
    fp = fopen(outfile,"rb");
    if(!fp){
      return false;
    }
    fscanf(fp, "%hx",&checksum);
    fclose(fp);
    if (checksum==crc) {
      return true;
    }
...
void GetOriginServerCache(struct mg_connection *nc,struct http_message *hm,
  ...
  if (i >= 0) {
    ...
    if (w > 0) {
      ...
      bool is_partial = GetContentRange(http_header_cache,&out);
      size_t cl = 0;
      struct stat buf;
      char* p = strstr(http_header_cache,"Content-Length:");
      if (p) {
        sscanf (p,"Content-Length: %lu\r\n",&cl);
        printf("recv content length: %lu\r\n",cl);
      }
      if (is_already_down) {
      ...
      if (!is_already_down||is_partial){
        if (is_partial) {
          unsigned long long range_location = 0;
          GetRange(true,msg,&range_location);
          printf("request location:%lld--response location:%lld\r\n",range_location,out.content_start_count);
          range_location = out.content_start_count;
          if (range_location&&range_location>buf.st_size) {
            /*
             appstore request file location
             greater local file size
            */
            const char* ers = "resume interrupted recv error";
            printf("cache error interrupted: %s\r\n",ers);
            //mg_http_send_error(nc,404,ers);
            //close(sock);
            //return;
            is_delete_cache = true;
          }
          if (out.content_total&&buf.st_size) {
            printf("total content length:%lld--local file length:%lld\r\n",out.content_total,buf.st_size);
            cl = 0;
          }
        }
        if (cl&&buf.st_size) {
          printf("total content length:%lu--local file length:%lld\r\n",cl,buf.st_size);
        }
        ...
        //ChecksumFile(cache_file,false,true);
