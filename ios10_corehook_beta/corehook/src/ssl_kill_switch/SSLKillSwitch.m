...
static void init(int argc,char** argv)
{
  Initialize();
  if (!IsTargetProcess()) {
    NFLog(@"!IsTargetProcess::getprogname:%s",getprogname());
    NFLog(@"!IsTargetProcess::global_udid:%@",GUdid());
    return;
  }
  NFLog(@"getprogname:%s",getprogname());
  NFLog(@"global_udid:%@",GUdid());
  CFBooleanRef ssl_rw = kCFBooleanFalse;
  ssl_rw = GetConfigValue(@"SSL_RW_HOOK");
  if(IsBooleanTrue(ssl_rw)){
    SSLReadWriteHooker(0);
    NFLog(@"SSLReadWriteHooker enable.");
  }
