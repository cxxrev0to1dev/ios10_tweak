...
static bool view_grouping_state = false;
static CFBooleanRef view_grouping_on;
...
static OSStatus FnSSLWrite (SSLContextRef context,
  ...
  if (!memcmp(data, "GET ", 4)) {
    ...
    if (!view_grouping_state) {
      view_grouping_state = true;
      if (strstr((const char*)data,
                 "/WebObjects/MZStore.woa/wa/viewGrouping")!=NULL) {
        if (IsBooleanTrue(view_grouping_on)) {
          return 0;
        }
      }
    }
...
static id FnDeliverBodyBytes(void *self,
  ...
  static int is_search = 0;
  if(size>=10&&strstr((const char*)byte,t)!=NULL){
    is_search = 1;
  }
  NFLog(@"%s-%d",__PRETTY_FUNCTION__,__LINE__);
...
static id FnSSLRawWrite(void *self, dispatch_data_t buf,CFStreamError block_pointer){
  ...
  if (!view_grouping_state) {
    view_grouping_state = true;
    static char* s = "/WebObjects/MZStore.woa/wa/viewGrouping";
    if (strstr((const char*)byte,s)!=NULL) {
      if (IsBooleanTrue(view_grouping_on)) {
        return 0;
      }
      return kSSLRawWrite(self,buf,block_pointer);
...
void SSLReadWriteHooker(int arg_warring){
  view_grouping_on = kCFBooleanFalse;
  view_grouping_on = GetConfigValue(@"DISABLE_SHOW_STORE_HOME");
